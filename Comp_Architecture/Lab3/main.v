...
module EX_MEM_Fixed (
    ...
    output reg          reg_write_out,
    output reg          mem_to_reg_out,
    output reg [3:0]    funct_out  // <<-- [修正2] 新增输出
);
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            alu_result_out <= 32'b0;
...
module RegFile(
    ...
    // 组合逻辑读操作：立即反应
    // x0寄存器恒为0
    assign read_data1 = (rs1 != 5'b0) ? regs[rs1] : 32'b0;
    assign read_data2 = (rs2 != 5'b0) ? regs[rs2] : 32'b0;

    // 时序逻辑写操作：带有异步复位功能的标准写法
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            // 当复位信号为高电平时，将所有寄存器清零
            ...
            // 在时钟上升沿，如果RegWrite为1且目标不是x0，则写入数据
...
module riscv_pipeline_top(
    ...
    wire [3:0]  funct_id_ex; // funct3 & funct7[5]
    ...
    wire [3:0]  funct_ex_mem; // <<-- [修正2] 新增funct3/funct7的传递通道

    // MEM Stage
    wire [31:0] mem_read_data_mem;

    // MEM/WB
    wire [31:0] mem_data_mem_wb, alu_result_mem_wb;
    wire [4:0]  rd_addr_mem_wb;
    wire        reg_write_mem_wb, mem_to_reg_mem_wb;
    ...
    //================================================================
    PC pc_reg (
        .clk(clk), .reset(rst), .next_pc(pc_next), .pc(pc_current)
    );
    Adder pc_adder (
    ...
    IF_ID if_id_reg (
        .clk(clk), .rst(rst), .pc_in(pc_plus_4), .instr_in(instruction),
        .pc_out(pc_if_id), .instr_out(instr_if_id)
    );

    //================================================================
    // ID Stage - 指令译码和寄存器读取阶段
    //================================================================
    Control control_unit (
        .opcode(instr_if_id[6:0]), .Branch(branch_id), .MemRead(mem_read_id),
        .MemtoReg(mem_to_reg_id), .ALUOp(alu_op_id), .MemWrite(mem_write_id),
        .ALUSrc(alu_src_id), .RegWrite(reg_write_id), .Jump(jump_id)
    );
    RegFile reg_file (
        .clk(clk), .reset(rst), .RegWrite(reg_write_mem_wb),
        .rs1(instr_if_id[19:15]), .rs2(instr_if_id[24:20]), .rd(rd_addr_mem_wb),
        .write_data(write_back_data), .read_data1(rs1_data_id), .read_data2(rs2_data_id)
    );
    ImmGen imm_gen (
        .instr(instr_if_id), .imm(imm_id)
    );
    ID_EX id_ex_reg (
        .clk(clk), .rst(rst),
        ...
        .branch_in(branch_id), .jump_in(jump_id),
        ...
        .branch_out(branch_id_ex), .jump_out(jump_id_ex)
    );
    ...
    ALUControl alu_ctrl_unit (.alu_op(alu_op_id_ex), .funct3(funct_id_ex[2:0]), .funct7({3'b0, funct_id_ex[3], 3'b0}), .alu_ctrl(alu_ctrl_ex));
    ...
    // 立即数左移一位 (用于B-type和J-type指令)
    shift_left_1 imm_shifter (.i(imm_id_ex), .o(imm_shifted_ex));
    
    // 计算分支/跳转目标地址
    Adder branch_adder (.a(pc_id_ex - 4), .b(imm_shifted_ex), .sum(pc_branch_target));

    // ---- [修正1] 完整的条件分支判断逻辑 ----
    always @(*) begin
        if (branch_id_ex) begin // 仅当是分支指令时才判断
            case (funct_id_ex[2:0]) // 判断funct3
                3'b000: pc_src_ex = alu_zero_ex;         // beq: Z=1
                3'b001: pc_src_ex = ~alu_zero_ex;        // bne: Z=0
                3'b100: pc_src_ex = alu_result_ex[31];   // blt: N=1 (signed)
                3'b101: pc_src_ex = ~alu_result_ex[31];  // bge: N=0 (signed)
    ...
    // PC更新逻辑
    assign jump_target = (jump_id_ex && alu_op_id_ex == 2'b11) ? alu_result_ex : pc_branch_target; // 处理jalr和jal
    assign pc_next = jump_id_ex ? jump_target : (pc_src_ex ? pc_branch_target : pc_plus_4);

    // 注意：需要修改EX_MEM模块以传递funct3
    EX_MEM_Fixed ex_mem_reg (
        .clk(clk), .rst(rst),
        .alu_result_in(alu_result_ex), .rs2_data_in(rs2_data_id_ex),
        .rd_addr_in(rd_addr_id_ex),
        .mem_read_in(mem_read_id_ex), .mem_write_in(mem_write_id_ex),
        .reg_write_in(reg_write_id_ex), .mem_to_reg_in(mem_to_reg_id_ex),
        .funct_in(funct_id_ex), // <<-- [修正2] 传递funct3/funct7

        .alu_result_out(alu_result_ex_mem), .rs2_data_out(rs2_data_ex_mem),
        .rd_addr_out(rd_addr_ex_mem),
        .mem_read_out(mem_read_ex_mem), .mem_write_out(mem_write_ex_mem),
        .reg_write_out(reg_write_ex_mem), .mem_to_reg_out(mem_to_reg_ex_mem),
        .funct_out(funct_ex_mem) // <<-- [修正2] 输出funct3/funct7
    );
    ...
    DataMem data_mem (
        ...
        .funct3(funct_ex_mem[2:0]), // <<-- [修正2] 使用从EX_MEM传来的funct3
    ...
    MEM_WB mem_wb_reg (
        .clk(clk), .rst(rst),
        .mem_data_in(mem_read_data_mem), .alu_result_in(alu_result_ex_mem),
        .rd_addr_in(rd_addr_ex_mem), .reg_write_in(reg_write_ex_mem),
        .mem_to_reg_in(mem_to_reg_ex_mem), .jump_in(1'b0), // jump信号在WB阶段不再需要

        .mem_data_out(mem_data_mem_wb), .alu_result_out(alu_result_mem_wb),
    ...
    Mux write_back_mux (
        ...
        .s(mem_to_reg_mem_wb), .d(write_back_data)
    );
